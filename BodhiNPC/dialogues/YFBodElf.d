...
//First dialogue
CHAIN IF ~Global("YFFirstBodhiElfDialogue","GLOBAL",1)~ THEN YFBodElf YFBodhiElfJoins0
	@1 /*You are beginning another adventure, <CHARNAME>, and I would love to accompany you.*/
		DO ~SetGlobal("YFFirstBodhiElfDialogue","GLOBAL",2)~
...
END
	++ @3 /*Don't tell me that the great Bodhi-of-the-Fallen-Leaves is already bored.*/ GOTO YFBodhiElfJoins1
	++ @4 /*Is life there not what you wanted?*/ GOTO YFBodhiElfJoins1
	++ @5 /*Are you not as happy in Suldanessellar as before?*/ GOTO YFBodhiElfJoins1
	
APPEND YFBodElf IF ~~ THEN BEGIN YFBodhiElfJoins1
	...
	IF ~~ THEN GOTO YFBodElfJoinsAsk1
	END
END

CHAIN YFBodElf YFBodElfJoinsAsk1
	@12 /*It is undoubtedly time for me to pause so that you may ask a question.*/
END
	++ @13 /*I won't let you get away with this. I shall destroy you!*/ GOTO YFBodElfJoinsDestroy1
	++ @14 /*I do not wish you in my party, but I wish you luck.*/ GOTO YFBodElfJoinsLuck1
	+ ~!Race(Player1,ELF) !Race(Player1,HALF_ELF)~+ @15 /*You do realize that I am a <PRO_RACE>, right?*/ GOTO YFBodElfJoinsRace1
	++ @16 /*My friends are not food.*/ GOTO YFBodElfJoinsFriends1
	++ @17 /*You won't eat elves or half-elves?*/ GOTO YFBodElfEatElves1
	++ @18 /*We cured you of your vampirism. You don't need to eat people!*/ GOTO YFBodElfDontNeedEat1
	++ @19 /*I am a vegetarian.*/ GOTO YFBodElfJoinsVegetarian1
	++ @20 /*Won't you miss your sister, Queen Ellesime, if you go?*/ GOTO YFBodElfJoinsEllesime1
	++ @21 /*Why don't you consider non-elven intelligent races to be people?*/ GOTO YFBodElfJoinsPeople1
	++ @22 /*If you're not a vampire anymore, what good are you?*/ GOTO YFBodElfJoinsFighter1
	++ @23 /*I would love to have you with me.*/ GOTO YFBodElfJoinsGroup1

CHAIN YFBodElf YFBodElfJoinsDestroy1
	@24 /*Do you remember the poison I used on the ship to Spellhold?*/
END
	++ @25 /*That one you put in my food and...?*/ GOTO YFBodElfJoinsDestroy2
	++ @26 /*The poison that didn't affect me until your brother said...?*/ GOTO YFBodElfJoinsDestroy2
	
CHAIN YFBodElf YFBodElfJoinsDestroy2
	@27 /*Sleep!*/
		DO ~ActionOverride(Player1,PlayDead(45)) FadeToColor([20.0],0) Wait(1) DestroySelf() FadeFromColor([20.0],0)~
EXIT

CHAIN YFBodElf YFBodElfJoinsLuck1
	@28 /*Luck to you as well, little godling.*/ 
		DO ~EscapeArea()~ 
EXIT
		
APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsRace1
	SAY @29 /*You are a child of Bhaal, little godling. You are not food.*/
	IF ~~ THEN GOTO YFBodElfJoinsAsk1
	END
END

APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsFriends1
	SAY @30 /*Naturally, <CHARNAME>. It would be the height of rudeness for me to eat your... pets.*/
	IF ~~ THEN GOTO YFBodElfJoinsAsk1
	END
END

CHAIN YFBodElf YFBodElfEatElves1
	@31 /*I am no longer a monster, <CHARNAME>. I will not eat my own kind, nor any of the related races.*/ 
END
	++ @32 /*Makes sense.*/ GOTO YFBodElfJoinsAsk1
	++ @33 /*Humans and elves are related species. They can even have children together.*/ GOTO YFBodElfEatElves2
	
CHAIN YFBodElf YFBodElfEatElves2
...
END
	++ @36 /*If you say so.*/ GOTO YFBodElfJoinsAsk1
	++ @37 /*Have you eaten horse?*/ GOTO YFBodElfEatElves3
	
APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfEatElves3
	SAY @38 /*Horses are delicious.*/
	IF ~~ THEN GOTO YFBodElfJoinsAsk1
	END
END

APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfDontNeedEat1
...
APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsVegetarian1
...
APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsEllesime1
...
CHAIN YFBodElf YFBodElfJoinsPeople1
...
END
	++ @50 /*Yes, of course.*/ GOTO YFBodElfJoinsPeople2
	++ @51 /*No, I guess you're right.*/ GOTO YFBodElfJoinsAsk1
	++ @52 /*That's different! They don't look like people!*/ GOTO YFBodElfJoinsPeople3

APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsPeople2
...
APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsPeople3
	SAY @55 /*Are you certain? As they say, "Looks are in the eyes of the beholder."*/
	IF ~~ THEN GOTO YFBodElfJoinsAsk1
	END
END

APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsFighter1
	SAY @56 /*I am the greatest warrior in the city of Suldanessellar. Try me.*/
		IF ~~ THEN GOTO YFBodElfJoinsAsk1
	END
END

APPEND YFBodElf IF ~~ THEN BEGIN YFBodElfJoinsGroup1
