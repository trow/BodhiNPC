...
APPEND YFBodhij IF WEIGHT #-1 ~Global("YFBodhiMist","GLOBAL",1)~ THEN BEGIN YFbodhimisttalk1
SAY @0 /*No, no it's mine! This life is mine! [BODHIX41.WAV]*/
	= @1 /*<CHARNAME>, I flee. When I am whole I will find you, if you live.*/
	IF ~~ THEN DO ~ApplySpellRES("YFbomi25",Myself) TransformItem("YFbohair","YFbohar2") SetGlobal("YFBodhiMist","GLOBAL",2) SetGlobal("YFBodhiRemoved","GLOBAL",2) LeaveParty() EscapeAreaMove("AR0810",383,479,S)~ EXIT  //This is the big crypt in the graveyard. Placeholder for now
	END
END

APPEND YFBodhij IF WEIGHT #-1 ~Global("YFBodhiBurns","GLOBAL",1)~ THEN BEGIN YFbodhiburnstalk1
SAY @2 /*The sun, it burns! I will find you later.*/
	IF ~~ THEN DO ~ApplySpellRES("YFbomi25",Myself) TransformItem("YFbohair","YFbohar2") SetGlobal("YFBodhiBurns","GLOBAL",2) SetGlobal("YFBodhiRemoved","GLOBAL",2) LeaveParty() EscapeAreaMove("AR0810",383,479,S)~ EXIT
	END
END

//1st Hexxat Cloak Talk
CHAIN IF WEIGHT #-1 ~Global("YFCloakTalk","GLOBAL",1)~ THEN HEXXATJ YFHexxatWhines
@3 /*You are wearing my cloak.*/
DO ~SetGlobal("YFCloakTalk","GLOBAL",2)~
== YFBodhij @4 /*It was Dragomir's cloak, was it not? <CHARNAME> killed Dragomir which makes it <CHARNAME>'s cloak, and <CHARNAME> gave it to me.*/
== HEXXATJ @5 /*You must return it to me.*/
== YFBodhij @6 /*Must I? It was a treasured gift.*/
...
END
	...
	++ @12 /*Hexxat's right, Bodhi, you need to return it.*/ EXTERN YFBodhij YFBodhiSad
	
CHAIN HEXXATJ YFSIHex
...
//2nd Hexxat Talk
CHAIN IF WEIGHT #-1 ~Global("YFCloakTalk","GLOBAL",5)~ THEN YFBodhij YFDoubleMintTwins
...
CHAIN IF ~Global("YFBodhiLoverDeadTalk","GLOBAL",1) Global("YFBodhiLoverDeadMale","GLOBAL",1)~ THEN YFBodhij YFBodhiDeadLoverMaleTalk1
	@18 /*Your sweet love is dead. Shall I raise him back to be your mindless slave forever?*/
		DO ~SetGlobal("YFBodhiLoverDeadTalk","GLOBAL",2)~
END
	++ @19 /*Tell me how to cure him!*/ GOTO YFBodhiDeadLoverTalk2
	++ @20 /*I will find a way to restore him!*/ GOTO YFBodhiDeadLoverTalk2

CHAIN IF ~Global("YFBodhiLoverDeadTalk","GLOBAL",1) Global("YFBodhiLoverDeadFemale","GLOBAL",1)~ THEN YFBodhij YFBodhiDeadLoverFemaleTalk1
	@21 /*Your sweet love is dead. Shall I raise her back to be your mindless slave forever?*/
		DO ~SetGlobal("YFBodhiLoverDeadTalk","GLOBAL",2)~
END
	++ @22 /*Tell me how to cure her!*/ GOTO YFBodhiDeadLoverTalk2
	++ @23 /*I will find a way to restore her!*/ GOTO YFBodhiDeadLoverTalk2
...
CHAIN IF ~Global("YFBodhiLoverDeadTalk","GLOBAL",3)~ THEN YFBodhij YFBodhiDeadLoverBooks1
	@26 /*Ah yes, those are the books. That I had set a trap on their container completely slipped my mind.*/
		DO ~SetGlobal("YFBodhiLoverDeadTalk","GLOBAL",4)~
...
END
	++ @29 /*So we need your heart?*/ GOTO YFBodhiDeadLoverBooks2
	
CHAIN YFBodhij YFBodhiDeadLoverBooks2
	@30 /*Yes, but have no fear on that account. I shall remove it for you and then regenerate another. It turns out a heart is not fully necessary for my continued existence.*/ 
		DO ~GiveItemCreate("MISCBP",Player1,0,0,0) ApplySpellRES("YF7dam","YFBodhi")~
...
APPEND YFBodhij IF ~Global("YFBodhiHideoutElhanTalk","GLOBAL",1)~ THEN BEGIN YFBodhiAsksAboutPlansElhan1
	SAY @32 /*So what now, <CHARNAME>? Do I return to Elhan, bound and chained? Elhan is not your friend.*/
	IF ~~ THEN DO ~SetGlobal("YFBodhiHideoutElhanTalk","GLOBAL",2)~ GOTO YFElhanNotHappyToSeeBodhi1
	END
END

CHAIN YFBodhij YFElhanNotHappyToSeeBodhi1
	@33 /*And he will not be happy to see me.*/
END
	++ @34 /*Can't we use the Rhynn Lanthorn to just walk to Suldanesselar without him?*/ GOTO YFCNWantstoWalkSuldenesselar1
	++ @35 /*Happy or not, he must help us. Only we can stop Irenicus.*/ GOTO YFElhanNotHappyToSeeBodhi2
	++ @36 /*What's the deal with you and Elhan, anyway?*/ GOTO YFElhanNotHappyToSeeBodhi3
	++ @37 /*You are wearing no chains, Bodhi. You move freely.*/ GOTO YFBodhiNotChainedElhan1
	+~Global("REYFodhiSex","GLOBAL",1)~+ @38 /*Forget about Elhan. Remember the last time we were in your lair... you led me to your coffin... and we...*/ GOTO YFCNRemembersBodhiSex1
	+~!Global("REYFodhiSex","GLOBAL",1)~+ @39 /*Forget about Elhan. I have chased you across half the world and now I have you. Let's spend some time alone.*/ GOTO YFCNWantsBodhiSex1
	++ @40 /*Off we go then.*/ EXIT

APPEND YFBodhij IF ~~ THEN BEGIN YFCNWantstoWalkSuldenesselar1
	SAY @41 /*One could, but only if one were an elf of the city, and I... am not. No... we must go to Elhan.*/
	IF ~~ THEN GOTO YFElhanNotHappyToSeeBodhi1
	END
END

APPEND YFBodhij IF ~~ THEN BEGIN YFElhanNotHappyToSeeBodhi2
...
END
	++ @46 /*I remember it well. I wanted you then and I want you now.*/ GOTO YFCNRemembersBodhiSex2
	++ @47 /*And ever since then I've been chasing you. Now that I've finally caught you, I want my reward.*/ GOTO YFCNRemembersBodhiSex2
	++ @48 /*You must have enjoyed it a lot since we also did it about a hundred times on the ship to Spellhold...*/ GOTO YFCNRemembersBodhiSex2

CHAIN YFBodhij YFCNRemembersBodhiSex2
...
END
	...
	++ @57 /*But I do love you, Bodhi. I stormed Spellhold for you. I braved the dangers of the Underdark. I sacrificed my own sister's soul.*/ GOTO YFCNRemembersBodhiSex3
	
CHAIN YFBodhij YFCNRemembersBodhiSex3
...
END
	++ @62 /*Of course you would say that. Very well.*/ DO ~SetGlobal("YFBodhiVampireSex","GLOBAL",1)~ GOTO YFCNRemembersBodhiSex4
	++ @63 /*As you command, Mistress.*/ DO ~SetGlobal("YFBodhiVampireSex","GLOBAL",1)~ GOTO YFCNRemembersBodhiSex4
	++ @64 /*Nah, I'm not interested.*/ EXIT

CHAIN YFBodhij YFCNRemembersBodhiSex4
...
CHAIN YFBodhij YFCNWantsBodhiSex1
...
END
	...
	++ @68 /*I am <CHARNAME> and even without what your brother stole, I was mighty enough to defeat you.*/ GOTO YFCNWantsBodhiSex2
	
CHAIN YFBodhij YFCNWantsBodhiSex2
...
END
	++ @56 /*Oh, never mind then.*/ EXIT
	++ @57 /*But I do love you, Bodhi. I stormed Spellhold for you. I braved the dangers of the Underdark. I sacrificed my own sister's soul.*/ GOTO YFCNWantsBodhiSex3
	
CHAIN YFBodhij YFCNWantsBodhiSex3
	@61 /*All right then, I accept. But you must follow all of my commands exactly. Obsequiousness is the greatest aphrodisiac.*/
END
	++ @62 /*Of course you would say that. Very well.*/ DO ~SetGlobal("YFBodhiVampireSex","GLOBAL",1)~ GOTO YFCNRemembersBodhiSex4
	++ @63 /*As you command, Mistress.*/ DO ~SetGlobal("YFBodhiVampireSex","GLOBAL",1)~ GOTO YFCNRemembersBodhiSex4
...
//Post-Sex Dialogue
CHAIN IF ~Global("YFBodhiPostVampireSexTalk","GLOBAL",1)~ THEN YFBodhij YFBodhiPostSex0
	@71 /*Hmm...*/
		DO ~SetGlobal("YFBodhiPostVampireSexTalk","GLOBAL",2)~
END
	++ @72 /*So how was it for you?*/ GOTO YFBodhiPostSex1
	++ @73 /*Was that a good "hmm" or a bad "hmm"?*/ GOTO YFBodhiPostSex1
	++ @74 /*That was awesome!*/ GOTO YFBodhiPostSex1
	++ @75 /*That didn't work, did it?*/ GOTO YFBodhiPostSex1

APPEND YFBodhij IF ~~ THEN BEGIN YFBodhiPostSex1
	...
	IF ~~ THEN GOTO YFPostSexTopConv1 
	END
END

CHAIN YFBodhij YFPostSexTopConv1
	@79 /*It is enough to make one slaughter a small city in frustration.*/
END
	++ @80 /*Maybe a different partner?*/ GOTO YFBodhiPostSexPartner1
	++ @81 /*So we cure your vampirism!*/ GOTO YFBodhiPostSexVampire1
	++ @82 /*Perhaps we just need to try again!*/ GOTO YFBodhiPostSexRepeat1
	++ @83 /*So what will you do now?*/ GOTO YFBodhiPostSexPlans1
	
APPEND YFBodhij IF ~~ THEN BEGIN YFBodhiPostSexPartner1
...
APPEND YFBodhij IF ~~ THEN BEGIN YFBodhiPostSexVampire1
...
APPEND YFBodhij IF ~~ THEN BEGIN YFBodhiPostSexRepeat1
...
CHAIN YFBodhij YFBodhiPostSexPlans1
...
END
	++ @94 /*Have fun.*/ GOTO YFBodhiPostSexPlans2
	++ @95 /*Can I come? Killing people always makes me feel better too.*/ GOTO YFBodhiPostSexPlans3
	++ @96 /*Well, I suppose I did sign up for this. Enjoy but try to stay discreet.*/ GOTO YFBodhiPostSexPlans2
	
CHAIN YFBodhij YFBodhiPostSexPlans2
	@97 /*I hope to. Sleep and regain your strength. I shall return before you wake.*/
		DO ~RestParty()~ 
EXIT

CHAIN YFBodhij YFBodhiPostSexPlans3
...
CHAIN IF WEIGHT #-1 ~Global("YFc6ElhanBodhiTalk","GLOBAL",1)~ THEN c6Elhan2  YFBodhiC6ElhanTalk1
	@99 /*How could you side with the defiled one, <CHARNAME>? She cannot be trusted!*/
		DO ~SetGlobal("YFc6ElhanBodhiTalk","GLOBAL",2)~
== YFBodhij @100 /*You wound me with your harsh words, my love. I am so cold, will you not warm me in your arms? Am I not worthy of your sweet caresses?*/
END
	++ @101 /*Clearly there is more to this story than you have told me.*/ EXTERN c6Elhan2 YFBodhiC6ElhanTalk2
	++ @102 /*I demand to know what is going on!*/ EXTERN c6Elhan2 YFBodhiC6ElhanTalk2
	++ @103 /*You claimed to know nothing about Bodhi, but she certainly seems to know you. Talk!*/ EXTERN c6Elhan2 YFBodhiC6ElhanTalk2
	
CHAIN c6Elhan2 YFBodhiC6ElhanTalk2 
...
== YFBodhij @107 /**You* were not involved? I suppose in a sense you weren't. To be involved, one must have the spine to actually risk something.*/
...
== YFBodhij @112 /*Jon has destroyed your temple, stolen your city, and laid waste to your defenses and you're worried that I might frighten some weak-hearted citizens? How misguidedly touching.*/
...
== YFBodhij @116 /*So they will be saved the indignity of having to pretend they cannot see and hear me as Elhan does? How fortunate for them.*/
...
END
	...
	++ @119 /*You know I must have forgotten it in my other backpack. I'll be back.*/ GOTO YFCNForgotLanthorn

CHAIN c6Elhan2 YFCNForgotLanthorn
...
CHAIN IF WEIGHT #-1 ~PartyHasItem("C6LANTHO") Global("YFc6ElhanBodhiTalk","GLOBAL",2)~ THEN c6Elhan2 YFCNRememberedLanthorn
...
== YFBodhij IF ~IfValidForPartyDialog("YFBodhi")~ THEN @122 /*Are you serious, my love? Your ability to have a conversation as if I were not present is truly impressive. If only you put as much effort into more worthwhile pursuits.*/
